...
export async function newSketch() {
    ...
    if (fs.existsSync(sketchDir) || !fs.existsSync(iterationDir)) {
        return Log.error('Unable to create a sketch because necessary files are missing or corrupted.');
    }

    const iterate = getOptionValue('iterate');
    const templateDir = getTemplateURL(getOptionValue('template'));
    const sourceDir = iterate && iterationDir || templateDir; 
    
    createSketch(sketchDir, sourceDir);

    Log.success(`Sketch created succesfully: ${sketchDir}`); 
}
